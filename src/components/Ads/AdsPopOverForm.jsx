...
import ReCaptchaPopOver from "@/util/ReCaptchaPopOver";
// import ReCaptchaAds from "@/util/reCaptchaAds";
import axios from "axios";
import Image from "next/image";
import React, { useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";

const AdsPopOverForm = ({ id = "captcha1", isVisible = true }) => {
  ...
  return (
    <section className="adsbgContat wrapper-padding wrapper-padding py-5 md:py-8  text-[#101763] rounded-[15px] lg:rounded-[30px]">
      <h2 className="font-sora text-xl md:text-2xl font-semibold text-left capitalize">
        Book a free tech consult
      </h2>

      <form
        className="mt-3 md:mt-5 space-y-3 md:space-y-4"
      ...
      >
        ...
        >
          ...
          <button
            ...
            className="w-[229px] h-14 md:h-[59px] flex items-center justify-center bg-[#d81100] font-satoshi text-sm md:text-base font-medium leading-[154%] space-x-9 text-white rounded-lg mx-auto group hover:bg-[#101763] transition-all duration-500 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Send Request
            <Image
              ...
              height={30}
              sizes="100vw"
              className="w-6 md:w-[33px] h-6 md:h-[33px] ml-2 group-hover:translate-x-3 transition-all"
            />
          </button>
        </div>
      </form>
